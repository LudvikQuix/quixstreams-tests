...
(including the resolutions of OQ-2 and OQ-3).
**Code:** `dc-battery-sim/main.py` (496 lines, ~270 of code), `dc-battery-sim/lexicon.json`,
...
four setpoints) and **parameters** (fourteen model constants, twelve of them tunable) are
...
The mutable state is `cmd` (four input signals) and `params` (fourteen parameters plus the
...
(`main.py:328-332`); everything after that reads the copies. The rule that makes this hold is
mechanical: **no module-level tunable is read inside the loop**. `solve_dc_current` therefore
takes `r0` as an argument (`main.py:273`) instead of reading a global, because the function is
...
| `CHILLER_POWERS`, `HEATER_POWERS`, `DERATING_LUT` | Not in the lexicon, therefore not addressable from the wire. |
...
`_alpha1`/`_alpha2` and are rewritten by `_recompute_derived()` (`main.py:109-118`) after
...
The rebase lives in the loop, immediately after the snapshot (`main.py:337-343`), because
...
1. `is_command` (`main.py:203`) is a `filter` placed *before* the update step. It rejects any
...
2. `coerce` (`main.py:143`) and `apply_updates` (`main.py:169`) use explicit checks only and
...
`CHILLER_POWERS[5]` raising `KeyError` inside the daemon thread. The thread died,
...
2. **A total lookup in the loop.** `CHILLER_POWERS.get(setting, 0.0)` (`main.py:353`) — an
   out-of-range setting can no longer kill the thread regardless of how it got into `cmd`.
   The `COOLANT_TEMP` lower clamp now keys off `power_chiller > 0.0` rather than
   `chiller_setting > 0`, so a setting with no power-map entry cannot engage a clamp for a
   chiller that is not running. For the three valid settings the two conditions are identical.
3. **A supervisor around the thread.** `supervise_simulation` (`main.py:437`) is the thread
...
`PARAM_RANGES` dict — has fewer moving parts but duplicates fourteen ranges that the dashboard
...
a `[STARTUP]` block prints the lexicon path and the effective value of all 14 parameters plus
...
run_simulation loop, once per SAMPLE_TIME:
  ...
  with state_lock: setpoints = dict(cmd); p = dict(params); echo_due = _echo_due; _echo_due = False
        ...
        ├─ power maps: CHILLER_POWERS.get(...) / HEATER_POWERS.get(...)
        ├─ ocv_lookup(q_act)
...
state. It carries `signals` (the four setpoints) and `parameters` (all 14, with the private
...
| File | Change | Why |
|---|---|---|
| `dc-battery-sim/lexicon.json` | **new** | Copied verbatim from `dev-planning/parameter-contract/lexicon-battery.json`. 16 signals + 14 parameters. The service copy is the live one; if the two ever diverge, this one wins. Already shipped by the dockerfile's `COPY . .` (line 22) — there is no `.dockerignore`. |
| `dc-battery-sim/main.py` | rewritten in place, 233 → 496 lines | Everything in §2. Still one file: ~270 lines of code, under the 300-line split trigger. If it grows past that, the seam named in spec §6.5 is `params.py` (lexicon load + `coerce` + `apply_updates` + `_recompute_derived`) and nothing else. |
| `dc-battery-sim/app.yaml` | 3 variables added, 18 descriptions rewritten, 3 defaults changed | `LEXICON_PATH`, `LOG_LEVEL`, `APPLIED_ECHO_PERIOD_S` added. `R1` `0.0`→`0.1`, `R2` `0.0`→`0.05` (D2). `input`/`output` defaults moved to `dashboard-in`/`dashboard-out`. Fixed parameters prefixed "FIXED — deploy-time only"; the other twelve suffixed "Startup default; tunable live over `dashboard-in`" so the Portal tells the truth about which knobs are live. |
| `dc-battery-sim/README.md` | parameter table, RC prose, thermal section, Data Flow, Architecture | The table gains Tunable and Range columns; the input section is replaced by the nested envelope plus a rejection table; the output section gains `applied`; the architecture section describes `cmd` + `params` under `state_lock` and the thread supervisor. |
...
- **DCM (D3)** — out of scope here by design. The service never calls DCM; it reads its own
  ...
  change.
- **Root `quix.yaml`** — does not exist yet (CLAUDE.md §5). When it is created, the
  `dc-battery-sim` deployment block needs `value:` entries for the three new variables and for
  `input`/`output`, or the running deployment never receives them.
...
- **No uint/int/bool coverage in the lexicon.** All 14 parameters are `float` and the only
